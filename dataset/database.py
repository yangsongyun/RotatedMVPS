...
class GlossyRealDatabase(BaseDatabase):
    ...
    def get_ps(self, img_id):
        img = self.get_image(img_id)
        h, w = img.shape[:2]
        return np.zeros((h, w, 32), dtype=np.float32)
...
class RotateDatabase(BaseDatabase):
    def __init__(self, database_name):
        super().__init__(database_name)
        _, model_name = database_name.split('/')
        RENDER_ROOT='data/Rotate'
        self.root=f'{RENDER_ROOT}/{model_name}'
        self.img_num = len(glob.glob(f'{self.root}/*.pkl'))
        self.img_ids= [str(k) for k in range(self.img_num)]
        self.cams = [read_pickle(f'{self.root}/{k}-camera.pkl') for k in range(self.img_num)]
        self.scale_factor = 1.0
        feat_path = f'{self.root}/feature.npy'
        self.feature_data = np.load(feat_path) if os.path.isfile(feat_path) else None
    ...
    def get_rotate(self,img_id):
        rotate = self.cams[int(img_id)][2]
        return rotate.astype(np.float32)
    ...
    def get_ps(self, img_id):
        ...
        if self.feature_data is None:
            return np.zeros((h, w, 32), dtype=np.float32)
        num_views = self.feature_data.shape[0]
        idx = int(img_id) % num_views
        ps = np.asarray(self.feature_data[idx], dtype=np.float32)
        if ps.shape[:2] != (h, w):
            raise ValueError(
                f'feature.npy spatial shape {ps.shape[:2]} does not match image ({h}, {w})'
            )
        if ps.shape[-1] != 32:
            raise ValueError(
                f'feature.npy last dim is {ps.shape[-1]}, expected 32 (see renderer feature_size)'
            )
        return ps
...
class CustomDatabase(BaseDatabase):
    ...
    def _parse_colmap(self):
        cache_path = Path(f'{self.root}/cache.pkl')
        if cache_path.is_file():
            self.poses, self.Ks, self.image_names, self.img_ids, self.rotate = read_pickle(
                str(cache_path)
            )
            return
        cameras, images, points3d = read_model(f'{self.root}/colmap/sparse/0')
        #关于scale的改动逻辑是这样的，由于在代码里面表示的是 pose.t, 在考虑了normal之后，R = R @ R_rec.T， rec@pose.t
        #因此新的公式变为rotate.t@rec@pose.t
        #在代码变动之后。理想情况应该对rotate进行变动，使得rotate变为rotate = rotate@R_rec.T
        self.poses, self.Ks, self.image_names, self.rotate, self.img_ids = {}, {}, {}, {}, []
        for img_id, image in images.items():
            self.img_ids.append(img_id)
            self.image_names[img_id] = image.name
            ...
            R = image.qvec2rotmat()
            t = image.tvec
            ones = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]], dtype=np.float32)
            pose = np.concatenate([ones, t[:, None]], 1).astype(np.float32)
            self.poses[img_id] = pose
            self.rotate[img_id] = R
            cam_id = image.camera_id
            camera = cameras[cam_id]
            if camera.model == 'SIMPLE_RADIAL':
                f, cx, cy, _ = camera.params
        ...
        save_pickle(
            [self.poses, self.Ks, self.image_names, self.img_ids, self.rotate],
            str(cache_path),
        )
    ...
    def _normalize(self):
        ...
        # R_new = R @ R_rec.T    t_new = (t - R @ offset) * scale
        for img_id, pose in self.poses.items():
            R = self.rotate[img_id]
            t =pose[:,3]
            R_new = R @ R_rec.T
            t_new = (t - R @ offset) * scale
            ones = np.array([[1,0,0],[0,1,0],[0,0,1]],dtype=np.float32)
            self.poses[img_id] = np.concatenate([ones, t_new[:,None]], -1)
            self.rotate[img_id] = R_new
    ...
    def get_mask(self, img_id):
        mask = imread(f'{self.root}/mask/{(img_id-1):03d}.png')
        resize_mask = resize_img(np.expand_dims(mask, axis=-1),self.ratio)
        resize_mask = resize_mask!=0
        return resize_mask
    ...
    def get_rotate(self, img_id):
        rotate = (self.rotate[img_id].copy())
        return rotate.astype(np.float32)
...
class RealRotateDatabase(BaseDatabase):
    def __init__(self, database_name):
        super().__init__(database_name)
        _, model_name = database_name.split('/')
        RENDER_ROOT='data/real'
        self.root=f'{RENDER_ROOT}/{model_name}'
        self.img_num = len(glob.glob(f'{self.root}/*.pkl'))
        self.img_ids= [str(k) for k in range(self.img_num)]
        self.cams = [read_pickle(f'{self.root}/{k}-camera.pkl') for k in range(self.img_num)]
    ...
    def get_image(self, img_id):
        return imread(f'{self.root}/{img_id}-mask-rgb.png')[...,:3]
    ...
    def get_mask(self, img_id):
        assert(self.scale_factor==1.0)
        mask = imread(f'{self.root}/{img_id}-mask.png')
        mask = mask!=0
        return mask
