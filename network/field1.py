...
class AppShadingNetwork(nn.Module):
    ...
    def predict_specular_lights(self, points, feature_vectors,rotate, reflective, roughness, human_poses, step):
        ...
        rotate_ref_roughness = self.sph_enc(rotate_reflective, roughness)
        ...
        #这里有一个问题，这里面的innerlight使用的是物体间反射所得出的innerlight，那么这个indirect_light应该指代的是物体的
        #groundtruth的reflectance，因此使用的reflective应该是等效的reflective（rotate@reflective）
        indirect_light = self.inner_light(torch.cat([pts, rotate_ref_roughness], -1))
        ref_ = self.dir_enc(rotate_reflective)
    ...
    def forward(self, points, normals, view_dirs,rotate, feature_vectors, human_poses, inter_results=False, step=None):
        #世界的normals与view_dirs和point
        ...
        specular_light, occ_prob, indirect_light, human_light = self.predict_specular_lights(points, feature_vectors,rotate, reflective, roughness, human_poses, step)
    ...
    def env_light(self, h, w, gamma=True):
        ...
        for ri in range(0, h*w, batch_size):
            with torch.no_grad():
                ide = self.sph_enc(xyzs,0)
                light = self.outer_light(ide[ri:ri+batch_size])
            lights.append(light)
        if gamma:
...
class MCShadingNetwork(nn.Module):
    ...
    def predict_outer_lights(self, points,rotate, directions):
        rotate_directions = torch.matmul(rotate,directions.unsqueeze(-1)).squeeze(-1)
        if self.cfg['outer_light_version'] == 'direction':
            outer_enc = self.sph_enc(rotate_directions, 0)
            outer_lights = self.outer_light(outer_enc)
        elif self.cfg['outer_light_version'] == 'sphere_direction':
            outer_dirs = rotate_directions
            outer_pts = points
    ...
    def get_lights(self, points, directions,rotate, human_poses):
        # trace
        ...
        if torch.sum(miss_mask)>0:
            outer_lights = self.predict_outer_lights(points[miss_mask],rotate[miss_mask], directions[miss_mask])
            if self.cfg['human_lights']:
    ...
    def shade_mixed(self, pts, normals, view_dirs, reflections,rotate, metallic, roughness, albedo, human_poses, is_train):
        #这个是渲染方程，此时已知mesh了，这个时候的输入是等效的normal，view——dirs，reflections。
        #此时在预测outlight的时候，应该增加改变使得对于outlight的环境光是实际上的位姿
        ...
        rotate_ = rotate.unsqueeze(1).repeat(1, sn, 1,1)
        lights, hl, light_pts, light_normals, light_pts_mask = self.get_lights(pts_, directions,rotate_, human_poses) # pn,sn,3
    ...
    def forward(self, pts, view_dirs, normals, human_poses,rotate, step, is_train):
        #在这里对normal与reflection进行操作，对normal与view_dir进行更改
        #这样的话normal和view_dir的方向是和实际方向一致的
        #rotate_normals = torch.matmul(rotate,normals.unsqueeze(-1)).squeeze(-1)
        #rotate_view_dirs = torch.matmul(rotate,view_dirs.unsqueeze(-1)).squeeze(-1)
        ...
        return self.shade_mixed(pts, normals, view_dirs, reflections,rotate, metallic, roughness, albedo, human_poses, is_train)
