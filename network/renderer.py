...
def build_imgs_info(database:BaseDatabase, img_ids):
    ...
    rotate = [database.get_rotate(img_id) for img_id in img_ids]
    ps = [database.get_ps(img_id) for img_id in img_ids]

    images = np.stack(images, 0)
    images = color_map_forward(images).astype(np.float32)
    Ks = np.stack(Ks, 0).astype(np.float32)
    poses = np.stack(poses, 0).astype(np.float32)
    rotate = np.stack(rotate, 0).astype(np.float32)
    ps = np.stack(ps, 0).astype(np.float32)

    return {'imgs': images, 'Ks': Ks, 'poses': poses,'rotate':rotate,'ps':ps}

def imgs_info_to_torch(imgs_info, device='cpu'):
    for k, v in imgs_info.items():
        v = torch.from_numpy(v)
        if k.startswith('imgs') or k.startswith('ps'): v = v.permute(0,3,1,2)
...
def imgs_info_downsample(imgs_info, ratio):
    ...
    imgs_info_copy['imgs'], imgs_info_copy['Ks'], imgs_info_copy['ps']= [], [],[]

    for bi in range(b):
        img = imgs_info['imgs'][bi].cpu().numpy().transpose([1,2,0])
        feature = imgs_info['ps'][bi].cpu().numpy().transpose([1,2,0])
        img = downsample_gaussian_blur(img, ratio)
        feature = downsample_gaussian_blur(feature, ratio)
        img = cv2.resize(img, (dw,dh), interpolation=cv2.INTER_LINEAR)
        feature = cv2.resize(feature, (dw,dh), interpolation=cv2.INTER_LINEAR)
        imgs_info_copy['imgs'].append(torch.from_numpy(img).permute(2,0,1))
        imgs_info_copy['ps'].append(torch.from_numpy(feature).permute(2,0,1))
    ...
    imgs_info_copy['ps'] = torch.stack(imgs_info_copy['ps'], 0)
...
class NeROShapeRenderer(nn.Module):
    ...
    def _init_dataset(self):
        ...
        self.train_batch, self.train_poses,self.inv_train_rotate, self.tbn, _, _ = self._construct_ray_batch(self.train_imgs_info)
    ...
    def _construct_ray_batch(self, imgs_info, device='cpu'):
        ...
        #这个是批量读取之前在database里面的数据集，在这里面增加rotate数据
        #由于读取的数据下，相机的等效位姿invers（rotate）因此从这里输入的就是imgsinfo的逆。
        ...
        inv_rotate = torch.inverse(imgs_info['rotate'])# imn,3,3
        feature_size = self.cfg['feature_size']
        ps = imgs_info['ps'].permute(0, 2, 3, 1).reshape(imn, h * w, feature_size)  # imn,h*w,3
        rn = imn * h * w
        ray_batch = {
            'dirs': dirs.float().reshape(rn, 3).to(device),
            'rgbs': imgs.float().reshape(rn, 3).to(device),
            'idxs': idxs.long().reshape(rn, 1).to(device),
            'feature': ps.float().reshape(rn, feature_size).to(device),

        }
        return ray_batch, poses,inv_rotate, rn, h, w
    ...
    def get_feature(self, feature):
        #理论上来说，这里面可以处理一下feature
        #feature 是一个384维度的函数，实际上我在思考这个有什么用呢



        return feature

    def _process_ray_batch(self, ray_batch, poses,inv_rotate):
        #这个是对rotate数据进行处理，这里面的光线数据需要乘以rotate转换
        #permute(0, 2, 1)是对矩阵的转置，所以这个是c2w矩阵（送入的是w2c矩阵）
        # r@c2w = new_c2w
        rays_d = ray_batch['dirs']  # rn,3
        idxs = ray_batch['idxs'][..., 0]  # rn

        rays_o = inv_rotate @ poses[:, :, :3].permute(0, 2, 1) @ -poses[:, :, 3:]  # trn,3,1
        rays_o = rays_o[idxs, :, 0]  # rn,3
        rays_d = inv_rotate[idxs] @ poses[idxs, :, :3].permute(0, 2, 1) @ rays_d.unsqueeze(-1)
        ...
        return rays_o, rays_d,near, far, human_poses[idxs],inv_rotate[idxs]  # rn, 3, 4
    ...
    def test_step(self, index, step,):
        ...
        ray_batch, input_poses,inv_rotate, rn, h, w = self._construct_ray_batch(imgs_info)

        input_poses = input_poses.float().cuda()
        inv_rotate = inv_rotate.float().cuda()
        ...
        outputs_keys += [
            ...
            'metallic', 'roughness', 'occ_prob', 'indirect_light', 'occ_prob_gt','feature'
        ]
        ...
        for ri in range(0, rn, trn):
            ...
            rays_o, rays_d, near, far, human_poses,inv_rotate = self._process_ray_batch(cur_ray_batch, input_poses,inv_rotate)
            cur_outputs = self.render(rays_o, rays_d, near, far, human_poses,inv_rotate, 0, 0, is_train=False, step=step)
        ...
        outputs['feature_loss'] = self.compute_rgb_loss(outputs['feature'], ray_batch['feature'])
    ...
    def train_step(self, step):
        ...
        inv_train_rotate = self.inv_train_rotate.cuda()
        rays_o, rays_d, near, far, human_poses, inv_rotate = self._process_ray_batch(train_ray_batch, train_poses,inv_train_rotate)

        outputs = self.render(rays_o, rays_d, near, far, human_poses,inv_rotate, -1, self.get_anneal_val(step), is_train=True, step=step)
        outputs['loss_rgb'] = self.compute_rgb_loss(outputs['ray_rgb'], train_ray_batch['rgbs'])  # ray_loss
        outputs['feature_loss'] = self.compute_rgb_loss(outputs['feature'], train_ray_batch['feature'])
    ...
    def render(self, rays_o, rays_d, near, far, human_poses,inv_rotate, perturb_overwrite=-1, cos_anneal_ratio=0.0, is_train=True, step=None):
        """
        :param rays_o: rn,3
        :param rays_d: rn,3
        :param near:   rn,1
        ...
        ret = self.render_core(rays_o, rays_d, z_vals, human_poses,inv_rotate, cos_anneal_ratio=cos_anneal_ratio, step=step, is_train=is_train)
        return ret

    def compute_validation_info(self, z_vals, rays_o, rays_d, rotate, weights, human_poses, step):
        depth = torch.sum(weights * z_vals, -1, keepdim=True)  # rn,
        points = depth * rays_d + rays_o  # rn,3
        #对于这个gradient，应该转到现实坐标（左乘totate）
        rotate_rays_d = torch.matmul(rotate,rays_d.unsqueeze(-1)).squeeze(-1)
        gradients = self.sdf_network.gradient(points)  # rn,3
        rotate_gradients = torch.matmul(rotate,gradients.unsqueeze(-1)).squeeze(-1)
        inner_mask = torch.norm(points, dim=-1, keepdim=True) <= 1.0
        outputs = {
            'depth': depth,  # rn,1
            'normal': ((F.normalize(rotate_gradients, dim=-1) + 1.0) * 0.5) * inner_mask,  # rn,3
        }

        feature_vector = self.sdf_network(points)[..., 1:]  # rn,f
        _, occ_info, inter_results = self.color_network(points, rotate_gradients, -F.normalize(rotate_rays_d, dim=-1), rotate, feature_vector, human_poses, inter_results=True, step=step)
        reflective = occ_info['reflective']
        rotate_reflective = torch.matmul(rotate,reflective.unsqueeze(-1)).squeeze(-1)
        _, occ_prob, _ = get_intersection(self.sdf_inter_fun, self.deviation_network, points, rotate_reflective, sn0=128, sn1=9)  # pn,sn-1
    ...
    def compute_sdf_alpha(self, points, dists, dirs, rotate, cos_anneal_ratio, step):
        # points [...,3] dists [...] dirs[...,3]
        sdf_nn_output = self.sdf_network(points)
        sdf = sdf_nn_output[..., 0]
        feature_vector = sdf_nn_output[..., 1:]

        #这里通过sdf得出的gradient信息实际上就是normal结果。
        #gradients_rotate是gt世界gradients_rotate
        gradients = self.sdf_network.gradient(points)  # ...,3
        inv_s = self.deviation_network(points).clip(1e-6, 1e6)  # ...,1
        inv_s = inv_s[..., 0]
        gradients_rotate = torch.matmul(rotate,gradients.unsqueeze(-1)).squeeze(-1)

        if self.cfg['freeze_inv_s_step'] is not None and step < self.cfg['freeze_inv_s_step']:
            inv_s = inv_s.detach()

        true_cos = (dirs * gradients_rotate).sum(-1)  # [...]
        ...
        return alpha, gradients_rotate, feature_vector, inv_s, sdf
    ...
    def compute_occ_loss(self, occ_info, points, rotate, sdf, gradients, dirs, step):
        ...
        '''根据输入分析，occ_info是由specular计算出来的，根据物体内部的计算出来的occ_info，使用的是rotate信息，
        此时，occ_info, points, sdf使用的都是物体内部信息。而gradients，和reflective是计算的物体的外部信息，因此在计算时需要转
        回来。'''
        #这里是计算相交的点，计算inner_mask点没有偏移，计算了gt inner_mask
        #sdf_mask sdfmask来自于sdf函数，也是无偏差的
        
        inner_mask = torch.norm(points, dim=-1) < 0.999  # pn
        sdf_mask = torch.abs(sdf) < self.cfg['occ_sdf_thresh']
        normal_mask = torch.sum(rotate_gradients * dirs, -1) < 0  # pn
        ...
        if torch.sum(mask) > 0:
            inter_dist, inter_prob, inter_sdf = get_intersection(self.sdf_inter_fun, self.deviation_network, points[mask], rotate_reflective[mask], sn0=64, sn1=16)  # pn,sn-1
            occ_prob_gt = torch.sum(inter_prob, -1, keepdim=True)
            return F.l1_loss(occ_prob[mask], occ_prob_gt)
        else:
            return torch.zeros(1)

    def render_core(self, rays_o, rays_d, z_vals, human_poses,inv_rotate, cos_anneal_ratio=0.0, step=None, is_train=True):
        batch_size, n_samples = z_vals.shape

        #这个是render方式，其中有两点需要改动，对于reflective的渲染方式，我们应该将物体转回去（包括了dirs与normal）
        #对于外和内都需要转回去,一方面，需要转变实际的观察方向dir，另一方面，需要旋转sdf拟合的normal结果。
        ...
        #计算真正的观测方向
        ##这里做一个测试，测试一下是否有效（rotate）
        rotate = inv_rotate.transpose(-2, -1)
        #rotate =inv_rotate
        _rotate = rotate
        rotate_rays_d = torch.matmul(rotate,rays_d.unsqueeze(-1))
        rotate_rays_d = rotate_rays_d.squeeze(-1)
        #转回gt
        dirs = rotate_rays_d.unsqueeze(-2).expand(batch_size, n_samples, 3)
        rotate = rotate.unsqueeze(-3).expand(batch_size, n_samples, 3, 3)
        ...
        feature = torch.zeros(batch_size, n_samples,self.cfg['feature_size'])
        ...
        if torch.sum(inner_mask) > 0:
            alpha[inner_mask], gradients, feature_vector, inv_s, sdf = self.compute_sdf_alpha(points[inner_mask], dists[inner_mask], dirs[inner_mask],rotate[inner_mask], cos_anneal_ratio, step)
            #这里面返回的gradient是旋转的gradient，即gt世界里面的gradient
            sampled_color[inner_mask], occ_info = self.color_network(points[inner_mask], gradients, -dirs[inner_mask], rotate[inner_mask], feature_vector, human_poses_pt[inner_mask], step=step)
            # Eikonal loss
        ...
        feature[inner_mask] = self.sdf_network.get_feature(points[inner_mask])
        feature_sum = (feature * weights[..., None]).sum(dim=1)
        ...
        if self.cfg['apply_occ_loss']:
            # occlusion loss
            if torch.sum(inner_mask) > 0:
                outputs['loss_occ'] = self.compute_occ_loss(occ_info, points[inner_mask],rotate[inner_mask], sdf, gradients, dirs[inner_mask], step)
            else:
                outputs['loss_occ'] = torch.zeros(1)

        if not is_train:
            outputs.update(self.compute_validation_info(z_vals, rays_o, rays_d, _rotate, weights, human_poses, step))
...
class NeROMaterialRenderer(nn.Module):
    ...
    def _construct_ray_batch(self, imgs_info, device='cpu', is_train=True):
        ...
        #这里需要对raysd与rayso进行改动，这样获得的pose是等效的pose
        rotate = imgs_info["rotate"]  #imn,3,3
        inv_rotate = rotate.transpose(-2, -1)  #imn,3,3
        # imn,h*w,3 @ imn,3,3 => imn,h*w,3
        rays_d = coords @ torch.inverse(imgs_info['Ks']).permute(0, 2, 1)
        poses = imgs_info['poses']  # imn,3,4
        R, t = poses[:, :, :3], poses[:, :, 3:]
        R = torch.matmul(R,rotate)
        rays_d = rays_d @ R
        rays_d = F.normalize(rays_d, dim=-1)
        rays_o = -R.permute(0,2,1) @ t # imn,3,3 @ imn,3,1
        #rays_o = inv_rotate @ rays_o
        #rays_d = inv_rotate @ rays_d

        self._warn_ray_tracing(rays_o)
        rays_o = rays_o.permute(0, 2, 1).repeat(1, h*w, 1) # imn,h*w,3
        inters, normals, depth, hit_mask = self.trace_in_batch(rays_o.reshape(-1,3), rays_d.reshape(-1,3), cpu=True) # imn
        inters, normals, depth, hit_mask = inters.reshape(imn,h*w,3), normals.reshape(imn,h*w,3), depth.reshape(imn,h*w,1), hit_mask.reshape(imn, h*w)
        ...
        rotate = rotate.unsqueeze(1).repeat(1,h*w,1,1) # imn,h*w,3,4
        ...
        if is_train:
            ray_batch={
                ...
                'rotate':rotate[hit_mask].to(device),
    ...
    def shade(self, pts, view_dirs, normals, human_poses,rotate, is_train, step=None):
        rgb_pr, outputs = self.shader_network(pts, view_dirs, normals, human_poses, rotate, step, is_train)
    ...
    def train_step(self, step):
        ...
        view_dirs = -self.train_batch['rays_d'][self.train_batch_i:self.train_batch_i + rn].cuda() # view directions are opposite to ray directions
        ...
        rotate = self.train_batch['rotate'][self.train_batch_i:self.train_batch_i + rn].cuda()


        shade_outputs = self.shade(pts, view_dirs, normals, human_poses,rotate, True, step)
    ...
    def test_step(self, index):
        ...
        output_keys = {'rgb_gt':3, 'rgb_pr':3, 'specular_light':3, 'specular_color':3, 'diffuse_light':3, 'diffuse_color':3, 'albedo':3, 'metallic':1, 'roughness':1}
        ...
        for ri in range(0, rn, trn):
            ...
            if torch.sum(hit_mask)>0:
                ...
                rotate = ray_batch['rotate'][ri:ri+trn][hit_mask]
                
                shade_outputs = self.shade(pts, view_dirs, normals, human_poses,rotate, False)
